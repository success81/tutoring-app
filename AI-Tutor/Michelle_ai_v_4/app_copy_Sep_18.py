...
"""
        ...
        return jsonify({
            ...
            "session_complete": social_skills_interaction_count >= 17
        })
...
Please provide a comprehensive analysis that will help Michelle strengthen her fundamental skills in this area."""
    ...
    selected_quotes = random.sample(MOTIVATIONAL_QUOTES, 40)
    chosen_quotes = random.sample(selected_quotes, 3)
    quotes_prompt = f"""Analyze the following 3 motivational quotes in the context of Michelle's tutoring session:
1. "{chosen_quotes[0]}"
2. "{chosen_quotes[1]}"
3. "{chosen_quotes[2]}"
For each quote:
1. Explain how it relates to Michelle's learning experience and the challenges she faced during the tutoring session.
2. Provide 2-3 specific tips or exercises that can help Michelle embody the wisdom of the quote in her learning journey.
3. Describe how embracing the quote's message can contribute to Michelle's personal growth and academic success.
Topic: {topic}
Problem/Question: {question}
**Tutoring Session:**
{conversation}
Please provide an inspiring and encouraging analysis that will help Michelle build confidence and embrace personal growth."""
    ...
    performance_evaluation = basic_prompt(evaluation_prompt_part1, model="qwen2")
    fundamentals_evaluation = basic_prompt(evaluation_prompt_part2, model="qwen2")
    motivational_analysis = basic_prompt(quotes_prompt, model="qwen2")
