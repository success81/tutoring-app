...
def submit_for_evaluation():
    ...
    # Generate the first evaluation prompt for performance assessment and tips
...
Please format your response with clear headings and subheadings for each section and subsection. Provide a thorough and insightful analysis that will be valuable for Michelle's educational development."""
    ...
    # Generate the second evaluation prompt for fundamental skills analysis
    fundamentals_prompt = f"""Analyze the fundamental skills required to solve the problem in the following tutoring session with Michelle. Your analysis should include:

1. **Identification of Fundamental Skills:**
   - List and explain the core skills needed to understand and solve the problem presented in the session.
   - Describe how these skills relate to the specific topic and problem.

2. **Assessment of Michelle's Fundamental Skills:**
   - Evaluate how well Michelle demonstrated these fundamental skills during the session.
   - Identify areas where Michelle excelled and areas that need improvement.

3. **Exercises and Tips for Improvement:**
   - Provide at least 10 specific exercises or activities that Michelle can do to improve her fundamental skills.
   - For each exercise, explain how it relates to the core skills and how it will help Michelle improve.
   - Include a mix of exercises that target different learning styles (visual, auditory, kinesthetic).
...
Please provide a comprehensive analysis that will help Michelle strengthen her fundamental skills in this area."""
    ...
    # Generate the third evaluation prompt for confidence building
    confidence_prompt = f"""Analyze the tutoring session with Michelle and provide a confidence-building evaluation. Your analysis should include:

1. **Quote Selection and Analysis:**
   - Select 4 motivational quotes from the provided list that resonate with this tutoring session.
   - For each quote, explain how it relates to Michelle's learning experience and the challenges she faced.

2. **Confidence-Building Exercises:**
   - Provide at least 8 exercises or activities that can help Michelle build confidence in her abilities related to this topic.
   - For each exercise, explain how it can boost Michelle's confidence and how it relates to the selected quotes.

3. **Personal Growth Tips:**
   - Offer at least 6 tips for personal growth that align with the selected quotes and the topic of the tutoring session.
   - Explain how each tip can help Michelle develop a growth mindset and improve her overall learning experience.
...
Motivational Quotes:
{random.sample(MOTIVATIONAL_QUOTES, 50)}
...
Please provide an inspiring and encouraging analysis that will help Michelle build confidence and embrace personal growth."""
    ...
    performance_evaluation = basic_prompt(performance_prompt, model="qwen2")
    fundamentals_evaluation = basic_prompt(fundamentals_prompt, model="qwen2")
    confidence_evaluation = basic_prompt(confidence_prompt, model="qwen2")
