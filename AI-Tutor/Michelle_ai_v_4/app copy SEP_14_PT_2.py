...
9. If you believe Michelle has achieved the goals of the current scenario before reaching 20 interactions, offer to end the current scenario or continue with a related social skill.
...
# Helper functions will be included in the next part
...
def tutor():
    ...
    return render_template('tutor.html', topic=topic, question=display_question, context=context)
...
Human: {user_input}
AI: Respond to Michelle's input with a thoughtful comment or question to guide the conversation about social skills. 
If you believe the goals of the current scenario have been achieved, offer to end this scenario or continue with a related social skill.
Do not answer your own questions or continue the conversation without waiting for Michelle's response.
Additionally, if this is the 20th interaction or beyond, wrap up the session and ask if Michelle would like to practice more or end the session.
Current interaction count: {social_skills_interaction_count}"""

        ai_response = basic_prompt(full_prompt)
        
        # Check if the AI response indicates goal achievement or session wrap-up
        if "goals of the current scenario have been achieved" in ai_response.lower() or social_skills_interaction_count >= 20:
            return jsonify({"response": ai_response, "interaction_count": social_skills_interaction_count, "offer_continuation": True})
        else:
            return jsonify({"response": ai_response, "interaction_count": social_skills_interaction_count, "offer_continuation": False})
    elif topic == 'math':
...
@app.route('/end_or_continue_session', methods=['POST'])
def end_or_continue_session():
    global social_skills_interaction_count
    decision = request.json['decision']
    
    if decision == 'continue':
        social_skills_interaction_count = 0
        response = "Great! Let's continue practicing your social skills. What specific aspect of social interaction would you like to work on next? For example, we could focus on starting conversations, active listening, or handling disagreements politely."
    elif decision == 'new_scenario':
        social_skills_interaction_count = 0
        response = "Excellent! Let's move on to a new social scenario. Can you describe a social situation you'd like to practice or improve upon?"
    else:
        response = "Thank you for practicing your social skills today, Michelle. You've made great progress! Remember to apply what you've learned in your daily interactions. If you have any more questions or want to practice again, don't hesitate to come back!"
